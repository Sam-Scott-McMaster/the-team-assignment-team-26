...
rm -f *.gcda *.gcno *.gcov hangman gcov_output.txt hangman_score.txt
...
(echo -n "" | ./hangman) >> gcov_output.txt

#case 2: correct guesses
echo -e "\nCorrect Guesses Test Case:\n" >> gcov_output.txt
(echo -e "c\na\nn\nd\nEOF" | ./hangman) >> gcov_output.txt
...
gcov -b hangman-test.c >> gcov_output.txt
cat gcov_output.txt